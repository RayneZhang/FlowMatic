...
import { MathUtils, ShaderMaterial, Mesh, TextureLoader } from 'three';
...
const paletteMenu = AFRAME.registerComponent('palette-menu', {
        ...
        for (const pieceName of pieceNames) {
            ...
            switch (pieceName) {
                ...
                case 'prev': case 'next': case 'primitive': case 'sketchfab': case 'diagram': case 'text': case 'search-button': {
                    ...
                    iconEl.object3D.rotation.set(MathUtils.degToRad(-90), 0, 0);
                    pieceEl.setAttribute('material', 'color', inactiveColor);
                    break;
                }
    ...
    initRunStopLabel(): void {
        ...
        runLabel.object3D.rotation.x += MathUtils.degToRad(-90);
        stopLabel.object3D.rotation.x += MathUtils.degToRad(-90);
    ...
    initItemDescription(): void {
        ...
        // Set the description rotation.
        descripText.object3D.rotation.x += MathUtils.degToRad(-90);
        // Set the description position.
    ...
    loadModelInstance(_itemEl: any, _itemId: number): void {
        ...
        _itemEl.object3D.rotation.set(MathUtils.degToRad(-90), 0, 0);
        ...
        $.get(sketchfab.getUrl(), param, function (data, status, xhr) {
            if (status == 'success') {
                ...
                results.forEach((asset, i: number)=>{
                    ...
                    itemEl.object3D.rotation.set(MathUtils.degToRad(-90), 0, 0);
...
function hsv2rgb(hsv) {
    var r, g, b, i, f, p, q, t;
    var h = MathUtils.clamp(hsv.h, 0, 1);
    var s = MathUtils.clamp(hsv.s, 0, 1);
    var v = hsv.v;
